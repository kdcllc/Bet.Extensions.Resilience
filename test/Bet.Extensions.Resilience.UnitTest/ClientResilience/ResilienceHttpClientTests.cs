...
public class ResilienceHttpClientTests
{
    ...
    [Fact]
    public void When_The_Same_Type_Is_Added_Two_Copies_Of_HttpClient_Present()
    {
        // assign
        var services = new ServiceCollection();
        var builder = services.AddResilienceHttpClient<ITestClient, TestClient>();

        // once more of the same
        services.AddResilienceHttpClient<ITestClient, TestClient>();

        // Assert.Throws<InvalidOperationException>(() => );
        var sp = services.BuildServiceProvider();

        var clients = sp.GetServices(typeof(ITestClient));
